...
testIntMatrix2D(String name, IntMatrix2DTest t) {
  group(name, () {
    setUp(t.setUp);
    tearDown(t.tearDown);
...
abstract class IntMatrix2DTest {
  /** Matrix to test. */
  IntMatrix2D A;
  ...
  void testColumn() {
    IntMatrix1D col = A.column(A.columns ~/ 2);
  ...
  void testWhere() {
    ...
    IntMatrix2D B = A.where((IntMatrix1D element) {
  ...
  void testStrides() {
    int rowStride = 3;
    int colStride = 5;
    IntMatrix2D B = A.strides(rowStride, colStride);
    for (int r = 0; r < B.rows; r++) {
      for (int c = 0; c < B.columns; c++) {
