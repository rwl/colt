 ...
 *
 */
class SparseCCDComplexMatrix2D extends WrapperDComplexMatrix2D {
   ...
   *             .
   */
  factory SparseCCDComplexMatrix2D.fromValues(List<Float64List> values) {
    return new SparseCCDComplexMatrix2D.sized(values.length, values[0].length)
      ..setAll2D(values);
  }
   ...
   *            internal storage.
   */
  /*SparseCCDComplexMatrix2D(DZcs dzcs) {
    this(dzcs.m, dzcs.n);
    _rowIndexes = dzcs.i;
   ...
   */
  /*factory SparseCCDComplexMatrix2D.sized(int rows, int columns) {
   ...
   *             if <tt>rows<0 || columns<0</tt> .
   */
  factory SparseCCDComplexMatrix2D.sized(int rows, int columns, [int nzmax=null]) {
    if (nzmax == null) {
    ...
    return new SparseCCDComplexMatrix2D(rows, columns, rowIndexes, columnPointers, values);
  }
   ...
   *            numerical values
   */
  SparseCCDComplexMatrix2D(int rows, int columns, Int32List rowIndexes, Int32List columnPointers, Float64List values) : super(null) {
    try {
      _setUp(rows, columns);
    } on ArgumentError catch (exc) { // we can hold rows*columns>Integer.MAX_VALUE cells !
      if ("matrix too large" != exc.message) {
   ...
   */
  factory SparseCCDComplexMatrix2D.withValue(int rows, int columns, Int32List rowIndexes, Int32List columnIndexes, double re, double im, bool removeDuplicates) {
    /*try {
      _setUp(rows, columns);
    } on ArgumentError catch (exc) { // we can hold rows*columns>Integer.MAX_VALUE cells !
      if ("matrix too large" != exc.message) {
    ...
    final m = new SparseCCDComplexMatrix2D(rows, columns, _rowIndexes, _columnPointers, _values);
    if (removeDuplicates) {
      m.removeDuplicates();
    }
    return m;
   ...
   */
  factory SparseCCDComplexMatrix2D.withValues(int rows, int columns, Int32List rowIndexes, Int32List columnIndexes, Float64List values, bool removeDuplicates, bool removeZeroes) {
    /*try {
      _setUp(rows, columns);
    } on ArgumentError catch (exc) { // we can hold rows*columns>Integer.MAX_VALUE cells !
      if ("matrix too large" != exc.message) {
  ...
  DComplexMatrix2D forEach(final cfunc.DComplexDComplexFunction function) {
    if (function is cfunc.DComplexMult) { // x[i] = mult*x[i]
      ...
      for (int j = 0; j < nz; j++) {
        ...
        valE = DComplex.multiply(valE, alpha);
        valuesE[2 * j] = valE[0];
  ...
  DComplexMatrix2D copyFrom(DComplexMatrix2D source) {
    ...
    if (source is SparseCCDComplexMatrix2D) {
      SparseCCDComplexMatrix2D other = source;
    ...
    } else if (source is SparseRCDComplexMatrix2D) {
      SparseRCDComplexMatrix2D other = source.conjugateTranspose();
  ...
  DComplexMatrix2D forEachMatrix(final DComplexMatrix2D y, cfunc.DComplexDComplexDComplexFunction function) {
    ...
    if ((y is SparseCCDComplexMatrix2D) && (function == cfunc.plus)) { // x[i] = x[i] + y[i]
      SparseCCDComplexMatrix2D yy = y;
      int p,
          j,
          nz = 0,
          anz;
      ...
      /* get workspace */
      SparseCCDComplexMatrix2D C = new SparseCCDComplexMatrix2D.sized(m, n, anz + bnz);
      /* allocate result*/
    ...
    if (function is cfunc.DComplexPlusMultFirst) { // x[i] = alpha*x[i] + y[i]
      ...
      y.forEachNonZero((int i, int j, Float64List value) {
        set(i, j, DComplex.plus(DComplex.multiply(alpha, get(i, j)), value));
        return value;
      });
      return this;
    }
    ...
    if (function == cfunc.mult) { // x[i] = x[i] * y[i]
      ...
      for (int j = _columns; --j >= 0; ) {
        ...
        for (int k = columnPointersA[j + 1]; --k >= low; ) {
          ...
          valA = DComplex.multiply(valA, y.get(i, j));
          valuesA[2 * k] = valA[0];
    ...
    if (function == cfunc.div) { // x[i] = x[i] / y[i]
      ...
      for (int j = _columns; --j >= 0; ) {
        ...
        for (int k = columnPointersA[j + 1]; --k >= low; ) {
          ...
          valA[0] = valuesA[2 * k];
          valA[1] = valuesA[2 * k + 1];
          valA = DComplex.div_(valA, y.get(i, j));
          valuesA[2 * k] = valA[0];
          valuesA[2 * k + 1] = valA[1];
          if (valuesA[2 * k] == 0 && valuesA[2 * k + 1] == 0) _remove(i, j);
        }
   ...
   */
  SparseRCDComplexMatrix2D rowCompressed() {
    SparseCCDComplexMatrix2D tr = conjugateTranspose();
    SparseRCDComplexMatrix2D rc = new SparseRCDComplexMatrix2D.sized(_rows, _columns);
   ...
   */
  SparseCCDComplexMatrix2D transpose() {
  ...
  SparseCCDComplexMatrix2D conjugateTranspose() {
    int p, q, j, n, m;
    ...
    Ai = _rowIndexes;
    Ax = _values;
    SparseCCDComplexMatrix2D C = new SparseCCDComplexMatrix2D.sized(_columns, _rows, Ai.length);
    /* allocate result */
    w = new Int32List(m);
    /* get workspace */
    Cp = C._columnPointers;
  ...
  DComplexMatrix2D like2D(int rows, int columns) {
    return new SparseCCDComplexMatrix2D.sized(rows, columns);
  }
      ...
      /*if ((nthreads > 1) && (cardinality() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
        ...
        for (int i = 0; i < _columns; i++) {
          ...
          for (int k = columnPointersA[i]; k < high; k++) {
            ...
            valA[0] = valuesA[2 * k];
            valA[1] = valuesA[2 * k + 1];
            valA = DComplex.multiply(valA, yElem);
            valA = DComplex.multiply(valA, alpha);
            elementsZ[zeroZ + strideZ * j] += valA[0];
            elementsZ[zeroZ + strideZ * j + 1] += valA[1];
          }
        }
      ...
      /*if ((nthreads > 1) && (cardinality() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
        ...
        for (int i = 0; i < _columns; i++) {
          ...
          for (int k = _columnPointers[i]; k < high; k++) {
            ...
            valY[0] = elementsY[zeroY + strideY * _rowIndexes[k]];
            valY[1] = elementsY[zeroY + strideY * _rowIndexes[k] + 1];
            sum = DComplex.plus(sum, DComplex.multiply(valA, valY));
          }
          sum = DComplex.multiply(alpha, sum);
          valZ[0] = elementsZ[zidx];
          valZ[1] = elementsZ[zidx + 1];
          valZ = DComplex.multiply(valZ, beta);
          elementsZ[zidx] = sum[0] + valZ[0];
          elementsZ[zidx + 1] = sum[1] + valZ[1];
          zidx += strideZ;
        }
  ...
  DComplexMatrix2D multiply(DComplexMatrix2D B, DComplexMatrix2D C, [Float64List alpha = null, Float64List beta = null, bool transposeA = false, bool transposeB = false]) {
    if (alpha == null) {
      alpha = new Float64List.fromList([1.0, 0.0]);
    }
    if (beta == null) {
    ...
    bool ignore = (C == null);
    if (C == null) {
      if (B is SparseCCDComplexMatrix2D) {
        C = new SparseCCDComplexMatrix2D.sized(rowsA, p, (rowsA * p));
      } else {
    ...
    if ((B is DenseDComplexMatrix2D) && (C is DenseDComplexMatrix2D)) {
      ...
      if (transposeB) {
        BB = B.conjugateTranspose as DenseDComplexMatrix2D;
      } else {
        BB = B;
      }
      DenseDComplexMatrix2D CC = C;
      ...
      for (int jj = 0; jj < columnsB; jj++) {
        for (int kk = 0; kk < columnsA; kk++) {
          ...
          for (int ii = columnPointersA[kk]; ii < high; ii++) {
            ...
            valA = DComplex.multiply(valA, valB);
    ...
    } else if ((B is SparseCCDComplexMatrix2D) && (C is SparseCCDComplexMatrix2D)) {
      SparseCCDComplexMatrix2D AA;
      if (transposeA) {
        AA = conjugateTranspose();
      } else {
      ...
      SparseCCDComplexMatrix2D BB = B;
      if (transposeB) {
        BB = BB.conjugateTranspose();
      }
      SparseCCDComplexMatrix2D CC = C;
      int j,
          nz = 0,
          m,
          n;
      ...
      for (int i = _columns; --i >= 0; ) {
        ...
        for (int k = columnPointersA[i + 1]; --k >= low; ) {
          ...
          valA[0] = valuesA[2 * k];
          valA[1] = valuesA[2 * k + 1];
          fun.multiplicator = DComplex.multiply(valA, alpha);
          if (!transposeA) {
  ...
  int _scatter(SparseCCDComplexMatrix2D A, int j, Float64List beta, Int32List w, Float64List x, int mark, SparseCCDComplexMatrix2D C, int nz) {
    int i, p;
    Int32List Ap, Ai, Ci;
    Float64List Ax;
    ...
    for (p = Ap[j]; p < Ap[j + 1]; p++) {
      ...
      if (w[i] < mark) {
        ...
        if (x != null) {
          valA[0] = Ax[2 * p];
          valA[1] = Ax[2 * p + 1];
          valA = DComplex.multiply(beta, valA);
          x[2 * i] = valA[0];
          /* x(i) = beta*A(i,j) */
          x[2 * i + 1] = valA[1];
        }
