...
class DComplexFactory1D {
   ...
   */
  AbstractComplexVector append(AbstractComplexVector A, AbstractComplexVector B) {
    // concatenate
   ...
   * are copied.
   */
  AbstractComplexVector concat(List<AbstractComplexVector> parts) {
    if (parts.length == 0) {
      return make(0);
    }
   ...
   * (exclusive).
   */
  AbstractComplexVector random(int size) {
   ...
   * <tt>repeat</tt> times.
   */
  AbstractComplexVector repeat(AbstractComplexVector A, int repeat) {
    int size = A.length;
...
class DComplexFactory2D {
   ...
   * </pre>
   */
  AbstractComplexMatrix appendColumns(AbstractComplexMatrix A, AbstractComplexMatrix B) {
   ...
   * </pre>
   */
  AbstractComplexMatrix appendColumn(AbstractComplexMatrix A, AbstractComplexVector b) {
    // force both to have maximal shared number of rows.
    if (b.length > A.rows) {
      b = b.part(0, A.rows);
   ...
   *
   */
  AbstractComplexMatrix appendRows(AbstractComplexMatrix A, AbstractComplexMatrix B) {
   ...
   *
   */
  AbstractComplexMatrix appendRow(AbstractComplexMatrix A, AbstractComplexVector b) {
    // force both to have maximal shared number of columns.
   ...
   *             if the array is not rectangular.
   */
  static void _checkRectangularShape2D(List<List<AbstractComplexMatrix>> array) {
    int columns = -1;
    for (int r = 0; r < array.length; r++) {
      if (array[r] != null) {
        if (columns == -1) columns = array[r].length;
   ...
   */
  AbstractComplexMatrix compose(List<List<AbstractComplexMatrix>> parts) {
   ...
   */
  AbstractComplexMatrix composeDiagonal(AbstractComplexMatrix A, AbstractComplexMatrix B) {
    int ar = A.rows;
    int ac = A.columns;
    int br = B.rows;
    int bc = B.columns;
   ...
   */
  AbstractComplexMatrix composeDiagonal3(AbstractComplexMatrix A, AbstractComplexMatrix B, AbstractComplexMatrix C) {
   ...
   * from the given parts. Cells are copied.
   */
  AbstractComplexMatrix composeBidiagonal(AbstractComplexMatrix A, AbstractComplexMatrix B) {
    int ar = A.rows;
    int ac = A.columns;
    int br = B.rows;
    int bc = B.columns;
   ...
   * @return a new matrix.
   */
  AbstractComplexMatrix diag(AbstractComplexVector vector) {
   ...
   * @return a new vector.
   */
  AbstractComplexVector diagonal(AbstractComplexMatrix A) {
    int min = Math.min(A.rows, A.columns);
    AbstractComplexVector diag = _make1D(min);
    for (int i = 0; i < min; i++) {
   ...
   * elsewhere).
   */
  AbstractComplexMatrix identity(int rowsAndColumns) {
   ...
   */
  AbstractComplexVector _make1D(int size) {
   ...
   * (exclusive).
   */
  AbstractComplexMatrix random(int rows, int columns) {
   ...
   */
  AbstractComplexMatrix repeat(AbstractComplexMatrix A, int rowRepeat, int columnRepeat) {
    int r = A.rows;
    int c = A.columns;
