...
class DenseComplexMatrix extends ComplexMatrix {
   ...
   *             .
   */
  factory DenseComplexMatrix.fromList(List<Float64List> values) {
    return new DenseComplexMatrix(values.length, values.length == 0 ? 0 : values[0].length / 2)
      ..setAll2D(values);
  }
   ...
   *             .
   */
  factory DenseComplexMatrix.fromRealPart(DoubleMatrix realPart) {
    return new DenseComplexMatrix(realPart.rows, realPart.columns)..setReal(realPart);
  }
   ...
   */
  DenseComplexMatrix(int rows, int columns, [Float64List elements = null, int rowZero = 0, int columnZero = 0, int rowStride = null, int columnStride = 2, bool isNoView = true]) {
    if (rowStride == null) {
      rowStride = 2 * columns;
    }
    if (elements == null) {
  ...
  Float64List reduceMatrix(final ComplexMatrix other, final cfunc.ComplexComplexComplexFunction aggr, final cfunc.ComplexComplexComplexFunction f) {
    if (!(other is DenseComplexMatrix)) {
  ...
  ComplexMatrix forEachWhere(final cfunc.ComplexProcedure cond, final cfunc.ComplexComplexFunction function) {
    final int zero = index(0, 0);
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
      nthreads = Math.min(nthreads, _rows);
  ...
  ComplexMatrix forEachReal(final cfunc.ComplexRealFunction function) {
    final int zero = index(0, 0);
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
      nthreads = Math.min(nthreads, _rows);
  ...
  ComplexMatrix copyFrom(final ComplexMatrix source) {
    ...
    if (_haveSharedCells(other)) {
      ...
      if (!(c is DenseComplexMatrix)) { // should not happen
  ...
  ComplexMatrix forEachMatrix(final ComplexMatrix y, final cfunc.ComplexComplexComplexFunction function) {
    // overriden for performance only
    ...
    checkShape(y);
    final Float64List elemsOther = (y as DenseComplexMatrix)._elements;
    if (_elements == null || elemsOther == null) {
      throw new Error();
    }
  ...
  ComplexMatrix fill(final double re, final double im) {
    final int zero = index(0, 0);
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
      nthreads = Math.min(nthreads, _rows);
  ...
  ComplexMatrix setImaginary(final DoubleMatrix other) {
    ...
    final int zeroOther = other.index(0, 0);
    final int zero = index(0, 0);
    final Float64List elemsOther = (other as DenseDoubleMatrix).elements();
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
      nthreads = Math.min(nthreads, _rows);
      List<Future> futures = new List<Future>(nthreads);
  ...
  ComplexMatrix conjugateTranspose() {
    ComplexMatrix transpose = this.dice().copy();
    final Float64List elemsOther = (transpose as DenseComplexMatrix)._elements;
  ...
  DoubleMatrix imaginary() {
    final DenseDoubleMatrix Im = new DenseDoubleMatrix(_rows, _columns);
  ...
  DoubleMatrix real() {
    final DenseDoubleMatrix R = new DenseDoubleMatrix(_rows, _columns);
  ...
  ComplexVector vectorize() {
    final ComplexVector v = new DenseComplexVector(this.length);
    final int zero = index(0, 0);
  ...
  ComplexVector mult(final ComplexVector y, ComplexVector z, [Float64List alpha = null, Float64List beta = null, bool transposeA = false]) {
    ...
    ComplexVector zz;
    if (z == null) {
      zz = new DenseComplexVector(this._rows);
    } else {
      zz = z;
    }
  ...
  ComplexMatrix multiply(final ComplexMatrix B, ComplexMatrix C, [Float64List alpha = null, Float64List beta = null, final bool transposeA = false, final bool transposeB = false]) {
    if (alpha == null) {
      alpha = new Float64List.fromList([1.0, 0.0]);
    }
    if (beta == null) {
    ...
    if (C == null) {
      C = new DenseComplexMatrix(rowsC, columnsC);
    }
    ...
    for (int i = 0; i < noOfTasks; i++) {
      ...
      (AA as DenseComplexMatrix)._multiplySeq(BB, CC, alpha, beta, transposeA, transposeB);
    }
  ...
  ComplexMatrix _multiplySeq(ComplexMatrix B, ComplexMatrix C, Float64List alpha, Float64List beta, bool transposeA, bool transposeB) {
    if (transposeA) {
      return conjugateTranspose().multiply(B, C, alpha, beta, false, transposeB);
    }
    if (transposeB) {
  ...
  ComplexVector _like1D(int size, int zero, int stride) {
    return new DenseComplexVector(size, this._elements, zero, stride, false);
  }
  ...
  Object clone() {
    return new DenseComplexMatrix(_rows, _columns, _elements, _rowZero, _columnZero, _rowStride, _columnStride, _isNoView);
  }
}
