...
class DenseComplexVector extends ComplexVector {
  ...
  factory DenseComplexVector.fromList(Float64List values) {
    return new DenseComplexVector(values.length ~/ 2)
      ..setAll(values);
  }
   ...
   *             if <tt>size<0</tt>.
   */
  factory DenseComplexVector.fromRealPart(DoubleVector realPart) {
    return new DenseComplexVector(realPart.length)..setReal(realPart);
  }
   ...
   *             if <tt>size<0</tt>.
   */
  factory DenseComplexVector.fromParts(DoubleVector realPart, DoubleVector imaginaryPart) {
    return new DenseComplexVector(realPart.length)
        ..setReal(realPart)
   ...
   *             if <tt>size<0</tt>.
   */
  DenseComplexVector(int size, [Float64List elements = null, int zero = 0, int stride = 2, bool isNoView = true]) {
    if (elements == null) {
  ...
  Float64List reduceVector(final ComplexVector other, final cfunc.ComplexComplexComplexFunction aggr, final cfunc.ComplexComplexComplexFunction f) {
    if (!(other is DenseComplexVector)) {
  ...
  ComplexVector forEach(final cfunc.ComplexComplexFunction function) {
  ...
  ComplexVector forEachWhere(final cfunc.ComplexProcedure cond, final cfunc.ComplexComplexFunction function) {
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (_size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
      nthreads = Math.min(nthreads, _size);
      List<Future> futures = new List<Future>(nthreads);
  ...
  ComplexVector forEachReal(final cfunc.ComplexRealFunction function) {
    if (this._elements == null) {
      throw new Error();
    }
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
  ...
  ComplexVector copyFrom(ComplexVector source) {
    ...
    if (_haveSharedCells(other)) {
      ...
      if (!(c is DenseComplexVector)) { // should not happen
        return super.copyFrom(source);
      }
  ...
  ComplexVector forEachVector(ComplexVector y, final cfunc.ComplexComplexComplexFunction function) {
    if (!(y is DenseComplexVector)) {
      return super.forEachVector(y, function);
    }
    checkSize(y);
  ...
  ComplexVector setAll(Float64List values) {
    if (_isNoView) {
  ...
  DoubleVector imaginary() {
    final DenseDoubleVector Im = new DenseDoubleVector(_size);
  ...
  DoubleVector real() {
    final DenseDoubleVector R = new DenseDoubleVector(_size);
  ...
  ComplexMatrix reshape(final int rows, final int columns) {
    ...
    ComplexMatrix M = new DenseComplexMatrix(rows, columns);
  ...
  void swap(ComplexVector other) {
    if (!(other is DenseComplexVector)) {
      super.swap(other);
    }
    DenseComplexVector y = other as DenseComplexVector;
    if (y == this) {
      return;
    }
    checkSize(y);
  ...
  Object clone() {
    return new DenseComplexVector(_size, _elements, _zero, _stride, _isNoView);
  }
}
