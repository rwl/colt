...
class DenseIntMatrix2D extends IntMatrix2D {
  ...
  factory DenseIntMatrix2D.fromList(List<Int32List> values) {
    return new DenseIntMatrix2D(values.length, values.length == 0 ? 0 : values[0].length)..setAll2D(values);
  }
  ...
  int reduceWith(final IntMatrix2D other, final ifunc.IntIntFunction aggr, final ifunc.IntIntFunction f) {
  ...
  IntMatrix2D forEachWhere(final ifunc.IntProcedure cond, final ifunc.IntFunction function) {
    final int zero = index(0, 0);
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
      nthreads = Math.min(nthreads, _rows);
  ...
  IntMatrix2D forEachWithRange(final IntMatrix2D y, final ifunc.IntIntFunction function, Int32List rowList, Int32List columnList) {
    checkShape(y);
    final int size = rowList.length;
    final Int32List rowElements = rowList;//.elements();
    final Int32List columnElements = columnList;//.elements();
  ...
  IntMatrix2D like2D(int rows, int columns) {
  ...
  IntMatrix1D mult(final IntMatrix1D y, IntMatrix1D z, [final int alpha = 1, int beta = null, final bool transposeA = false]) {
    if (beta == null) {
      beta = z == null ? 1 : 0;
    }
    if (transposeA) {
  ...
  IntMatrix2D multiply(final IntMatrix2D B, IntMatrix2D C, [final int alpha = 1, int beta = null, final bool transposeA = false, final bool transposeB = false]) {
    if (beta == null) {
      beta = C == null ? 1 : 0;
    }
    ...
    */
    ...
    if (!(C is DenseIntMatrix2D)) return super.multiply(B, C, alpha, beta, transposeA, transposeB);
  ...
  IntMatrix2D _zMultSequential(IntMatrix2D B, IntMatrix2D C, int alpha, int beta, bool transposeA, bool transposeB) {
    if (transposeA) {
  ...
  IntMatrix2D _viewSelectionLike(Int32List rowOffsets, Int32List columnOffsets) {
    return new SelectedDenseIntMatrix2D.offset(this._elements, rowOffsets, columnOffsets, 0);
  }

  Object clone() {
    return new DenseIntMatrix2D(_rows, _columns, _elements, _rowZero, _columnZero, _rowStride, _columnStride, !_isNoView);
  }
}
...
class SelectedDenseIntMatrix2D extends IntMatrix2D {
   ...
   * @param offset
   */
  factory SelectedDenseIntMatrix2D.offset(Int32List elements, Int32List rowOffsets, Int32List columnOffsets, int offset) {
    return new SelectedDenseIntMatrix2D(rowOffsets.length, columnOffsets.length, elements, 0, 0, 1, 1, rowOffsets, columnOffsets, offset);
  }
   ...
   * @param offset
   */
  SelectedDenseIntMatrix2D(int rows, int columns, Int32List elements, int rowZero, int columnZero, int rowStride, int columnStride, Int32List rowOffsets, Int32List columnOffsets, int offset) {
  ...
  IntMatrix1D vectorize() {
    DenseIntMatrix1D v = new DenseIntMatrix1D(length);
    int idx = 0;
    for (int c = 0; c < _columns; c++) {
      for (int r = 0; r < _rows; r++) {
        v.set(idx++, get(c, r));
  ...
  IntMatrix1D column(int column) {
    ...
    return new SelectedDenseIntMatrix1D(viewSize, this._elements, viewZero, viewStride, viewOffsets, viewOffset);
  }
   ...
   */
  IntMatrix2D _viewSelectionLike(Int32List rowOffsets, Int32List columnOffsets) {
    return new SelectedDenseIntMatrix2D.offset(this._elements, rowOffsets, columnOffsets, this._offset);
  }

  Object clone() {
    return new SelectedDenseIntMatrix2D(_rows, _columns, _elements, _rowZero, _columnZero, _rowStride, _columnStride, _rowOffsets, _columnOffsets, _offset);
  }
}
