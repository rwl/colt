 ...
 *
 */
class DenseIntMatrix1D extends IntMatrix1D {
   ...
   */
  factory DenseIntMatrix1D.fromList(Int32List values) {
    return new DenseIntMatrix1D(values.length)..setAll(values);
  }
   ...
   *             if <tt>size<0</tt>.
   */
  /*factory DenseIntMatrix1D.sized(int size) {
        _setUp(size);
   ...
   *             if <tt>size<0</tt>.
   */
  DenseIntMatrix1D(int size, [Int32List elements = null, int zero = 0, int stride = 1, bool isView = false]) {
    if (elements == null) {
  ...
  int reduceWith(final IntMatrix1D other, final ifunc.IntIntFunction aggr, final ifunc.IntIntFunction f) {
    if (!(other is DenseIntMatrix1D)) {
  ...
  IntMatrix1D forEach(final ifunc.IntFunction function) {
    int multiplicator;
  ...
  IntMatrix1D forEachWhere(final ifunc.IntProcedure cond, final ifunc.IntFunction function) {
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (_size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
      nthreads = Math.min(nthreads, _size);
      List<Future> futures = new List<Future>(nthreads);
  ...
  IntMatrix1D fillWhere(final ifunc.IntProcedure cond, final int value) {
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (_size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
      nthreads = Math.min(nthreads, _size);
      List<Future> futures = new List<Future>(nthreads);
  ...
  IntMatrix1D fill(final int value) {
  ...
  IntMatrix1D setAll(final Int32List values) {
  ...
  IntMatrix1D copyFrom(IntMatrix1D source) {
    // overriden for performance only
    if (!(source is DenseIntMatrix1D)) {
      super.copyFrom(source);
      return this;
    }
    DenseIntMatrix1D other = source as DenseIntMatrix1D;
    ...
    if (_haveSharedCells(other)) {
      ...
      if (!(c is DenseIntMatrix1D)) {
        // should not happen
        super.copyFrom(source);
        return this;
      }
      other = c as DenseIntMatrix1D;
    }
  ...
  IntMatrix1D forEachWith(final IntMatrix1D y, final ifunc.IntIntFunction function) {
    // overriden for performance only
    if (!(y is DenseIntMatrix1D)) {
  ...
  IntMatrix2D reshape(final int rows, final int columns) {
  ...
  void swap(final IntMatrix1D other) {
    // overriden for performance only
    if (!(other is DenseIntMatrix1D)) {
      super.swap(other);
    }
    DenseIntMatrix1D y = other as DenseIntMatrix1D;
    if (y == this) {
      return;
    }
    checkSize(y);
  ...
  int dotProduct(IntMatrix1D y) {
    if (!(y is DenseIntMatrix1D)) {
      return super.dot(y);
    }
    DenseIntMatrix1D yy = y as DenseIntMatrix1D;
  ...
  int dot(IntMatrix1D y, [final int from = 0, int length = null]) {
    if (length == null) {
      length = _size;
    }
    if (!(y is DenseIntMatrix1D)) {
  ...
  bool _haveSharedCellsRaw(IntMatrix1D other) {
    if (other is SelectedDenseIntMatrix1D) {
      return this._elements == other._elements;
    } else if (other is DenseIntMatrix1D) {
      return this._elements == other._elements;
    }
    return false;
  }
  ...
  IntMatrix1D _viewSelectionLike(Int32List offsets) {
    return new SelectedDenseIntMatrix1D.offset(this._elements, offsets);
  }

  Object clone() {
    return new DenseIntMatrix1D(length, _elements, _zero, _stride, !_isNoView);
  }
}
...
class SelectedDenseIntMatrix1D extends IntMatrix1D {
   ...
   */
  factory SelectedDenseIntMatrix1D.offset(Int32List elements, Int32List offsets) {
    return new SelectedDenseIntMatrix1D(offsets.length, elements, 0, 1, offsets, 0);
  }
   ...
   * @param offset
   */
  SelectedDenseIntMatrix1D(int size, Int32List elements, int zero, int stride, Int32List offsets, int offset) {
    _setUp(size, zero, stride);
  ...
  IntMatrix2D reshape(int rows, int columns) {
    throw new ArgumentError("This method is not supported.");
  }
  ...
  Object clone() {
    return new SelectedDenseIntMatrix1D(length, _elements, _zero, _stride, _offsets, __offset);
  }
}
