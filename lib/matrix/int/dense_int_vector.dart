...
class DenseIntVector extends IntVector {
   ...
   *             if <tt>size<0</tt>.
   */
  DenseIntVector(int size, [Int32List elements = null, int zero = 0, int stride = 1, bool isView = false]) {
    if (elements == null) {
  ...
  IntVector forEach(final ifunc.IntFunction function) {
    int multiplicator;
  ...
  IntVector forEachWhere(final ifunc.IntProcedure cond, final ifunc.IntFunction function) {
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (_size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
      nthreads = Math.min(nthreads, _size);
      List<Future> futures = new List<Future>(nthreads);
  ...
  IntVector copyFrom(IntVector source) {
  ...
  IntMatrix reshape(final int rows, final int columns) {
    ...
    IntMatrix M = new DenseIntMatrix(rows, columns);
  ...
  void swap(final IntVector other) {
    ...
    DenseIntVector y = other as DenseIntVector;
    if (y == this) {
      return;
    }
    checkSize(y);
  ...
  Object clone() {
    return new DenseIntVector(length, _elements, _zero, _stride, !_isNoView);
  }
}
