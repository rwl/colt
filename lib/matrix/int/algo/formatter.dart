 ...
 * passed as format string, {@link java.lang.Int#toString2D(int)} is used instead,
...
class IntFormatter extends AbstractFormatter {
  ...
  String toSourceCode2D(IntMatrix matrix) {
    ...
    String lead = "{\n" + b3 + "{";
    String trail = "}\n};";
    return lead + copy.toString2D(matrix) + trail;
  }
  ...
  /*String toString3D(IntMatrix3D matrix) {
    ...
    for (int slice = 0; slice < matrix.slices(); slice++) {
      ...
      buf.append(toString2D(matrix.viewSlice(slice)));
    }
   ...
   * @return the matrix converted to a string.
   */
  /*String _xtoTitleString(IntMatrix3D matrix, List<String> sliceNames, List<String> rowNames, List<String> columnNames, String sliceAxisName, String rowAxisName, String columnAxisName, String title) {
    if (matrix.length == 0) {
      return "Empty matrix";
    }
    StringBuffer buf = new StringBuffer();
