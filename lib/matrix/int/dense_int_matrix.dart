 ...
 *
 */
class DenseIntMatrix extends IntMatrix {
   ...
   *             .
   */
  factory DenseIntMatrix.fromList(List<Int32List> values) {
    return new DenseIntMatrix(values.length, values.length == 0 ? 0 : values[0].length)..setAll2D(values);
  }
   ...
   */
  DenseIntMatrix(int rows, int columns, [Int32List elements = null, int rowZero = 0, int columnZero = 0, int rowStride = null, int columnStride = 1, bool isView = false]) {
    if (elements == null) {
  ...
  int reduceWith(final IntMatrix other, final ifunc.IntIntFunction aggr, final ifunc.IntIntFunction f) {
    if (!(other is DenseIntMatrix)) {
  ...
  IntMatrix forEach(final ifunc.IntFunction function) {
  ...
  IntMatrix forEachWhere(final ifunc.IntProcedure cond, final ifunc.IntFunction function) {
    final int zero = index(0, 0);
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
      nthreads = Math.min(nthreads, _rows);
  ...
  IntMatrix fillWhere(final ifunc.IntProcedure cond, final int value) {
    final int zero = index(0, 0);
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
      nthreads = Math.min(nthreads, _rows);
  ...
  IntMatrix fill(final int value) {
    final Int32List elems = this._elements;
    final int zero = index(0, 0);
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
  ...
  IntMatrix setAll(final Int32List values) {
  ...
  IntMatrix setAll2D(final List<Int32List> values) {
  ...
  IntMatrix copyFrom(final IntMatrix source) {
    // overriden for performance only
    if (!(source is DenseIntMatrix)) {
      super.copyFrom(source);
      return this;
    }
    final DenseIntMatrix other_final = source as DenseIntMatrix;
    ...
    DenseIntMatrix other = source as DenseIntMatrix;
    if (_haveSharedCells(other)) {
      IntMatrix c = other.copy();
      if (!(c is DenseIntMatrix)) { // should not happen
        super.copyFrom(other);
        return this;
      }
      other = c as DenseIntMatrix;
    }
  ...
  IntMatrix forEachWith(final IntMatrix y, final ifunc.IntIntFunction function) {
    // overriden for performance only
    if (!(y is DenseIntMatrix)) {
      super.forEachWith(y, function);
      return this;
    }
    DenseIntMatrix other = y as DenseIntMatrix;
    checkShape(y);
    final Int32List elemsOther = other._elements;
    if (_elements == null || elemsOther == null) {
      throw new Error();
  ...
  IntMatrix forEachWithRange(final IntMatrix y, final ifunc.IntIntFunction function, Int32List rowList, Int32List columnList) {
    checkShape(y);
    final int size = rowList.length;
    final Int32List rowElements = rowList;//.elements();
    final Int32List columnElements = columnList;//.elements();
  ...
  IntMatrix forEachNonZero(final ifunc.IntIntIntFunction function) {
    final int zero = index(0, 0);
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
      nthreads = Math.min(nthreads, _rows);
  ...
  IntVector vectorize() {
    final DenseIntVector v = new DenseIntVector(length);
    final int zero = index(0, 0);
  ...
  IntVector mult(final IntVector y, IntVector z, [final int alpha = 1, int beta = null, final bool transposeA = false]) {
    if (beta == null) {
      beta = z == null ? 1 : 0;
    }
    if (transposeA) {
  ...
  IntMatrix multiply(final IntMatrix B, IntMatrix C, [final int alpha = 1, int beta = null, final bool transposeA = false, final bool transposeB = false]) {
    if (beta == null) {
      beta = C == null ? 1 : 0;
    }
    ...
    if (C == null) {
      C = new DenseIntMatrix(rowsC, columnsC);
    }
    ...
    */
    ...
    if (!(C is DenseIntMatrix)) return super.multiply(B, C, alpha, beta, transposeA, transposeB);
  ...
  IntMatrix _zMultSequential(IntMatrix B, IntMatrix C, int alpha, int beta, bool transposeA, bool transposeB) {
    if (transposeA) {
  ...
  bool _haveSharedCellsRaw(IntMatrix other) {
    ...
    } else if (other is DenseIntMatrix) {
      return this._elements == other._elements;
    }
    return false;
  }
  ...
  IntVector _like1D(int size, int zero, int stride) {
    return new DenseIntVector(size, this._elements, zero, stride, true);
  }
  ...
  Object clone() {
    return new DenseIntMatrix(_rows, _columns, _elements, _rowZero, _columnZero, _rowStride, _columnStride, !_isNoView);
  }
}
 ...
 */
class SelectedDenseIntMatrix extends IntMatrix {
  ...
  IntVector vectorize() {
    DenseIntVector v = new DenseIntVector(length);
    int idx = 0;
    for (int c = 0; c < _columns; c++) {
      for (int r = 0; r < _rows; r++) {
        v.set(idx++, get(c, r));
