 ...
 *
 */
abstract class IntVector extends AbstractVector {
   ...
   * @see IntFunctions
   */
  int reduceWith(final IntVector other, final ifunc.IntIntFunction aggr, final ifunc.IntIntFunction f) {
    checkSize(other);
   ...
   * @see IntFunctions
   */
  IntVector forEach(final ifunc.IntFunction f) {
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (_size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
      nthreads = Math.min(nthreads, _size);
      List<Future> futures = new List<Future>(nthreads);
   ...
   * @see IntFunctions
   */
  IntVector forEachWhere(final ifunc.IntProcedure cond, final ifunc.IntFunction f) {
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (_size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
      nthreads = Math.min(nthreads, _size);
      List<Future> futures = new List<Future>(nthreads);
   ...
   *
   */
  IntVector fillWhere(final ifunc.IntProcedure cond, final int value) {
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (_size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
      nthreads = Math.min(nthreads, _size);
      List<Future> futures = new List<Future>(nthreads);
   ...
   * @return <tt>this</tt> (for convenience only).
   */
  IntVector fill(final int value) {
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (_size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
      nthreads = Math.min(nthreads, _size);
      List<Future> futures = new List<Future>(nthreads);
   ...
   */
  IntVector setAll(final Int32List values) {
   ...
   *             if <tt>size() != other.size()</tt>.
   */
  IntVector copyFrom(IntVector other) {
   ...
   * @see IntFunctions
   */
  IntVector forEachWith(final IntVector y, final ifunc.IntIntFunction function) {
    checkSize(y);
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (_size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
      nthreads = Math.min(nthreads, _size);
   ...
   * @see IntFunctions
   */
  IntVector forEachWithRange(IntVector y, ifunc.IntIntFunction function, List<int> nonZeroIndexes) {
    checkSize(y);
  ...
  bool operator ==(var obj) {
    ...
    return iprop.equalsVector(this, obj as IntVector);
  }
   ...
   *             if <tt>size() != other.size()</tt>.
   */
  void swap(final IntVector other) {
    checkSize(other);
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (_size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
      nthreads = Math.min(nthreads, _size);
   ...
   */
  IntVector flip() {
    return (_view()._vFlip()) as IntVector;
  }
  ...
  IntVector part(int index, int width) {
    return (_view()._vPart(index, width)) as IntVector;
  }
   ...
   * @return the new view.
   */
  IntVector where(ifunc.IntProcedure condition) {
  ...
  IntVector strides(int stride) {
    return (_view()._vStrides(stride)) as IntVector;
  }
   ...
   * @return the sum of products.
   */
  int dotNonZero(IntVector y, Int32List nonZeroIndexes, [int from = 0, int length = null]) {
    if (length == null) {
      length = _size;
    }
