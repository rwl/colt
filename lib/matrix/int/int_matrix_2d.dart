 ...
 *
 */
abstract class IntMatrix2D extends AbstractMatrix2D {
   ...
   * @see cern.jet.math.tint.IntFunctions
   */
  IntMatrix2D forEach(final ifunc.IntFunction f) {
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (_rows * _columns >= ConcurrencyUtils.getThreadsBeginN_2D())) {
      nthreads = Math.min(nthreads, _rows);
      List<Future> futures = new List<Future>(nthreads);
   ...
   * @see cern.jet.math.tint.IntFunctions
   */
  IntMatrix2D forEachWhere(final ifunc.IntProcedure cond, final ifunc.IntFunction f) {
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (_rows * _columns >= ConcurrencyUtils.getThreadsBeginN_2D())) {
      nthreads = Math.min(nthreads, _rows);
      List<Future> futures = new List<Future>(nthreads);
   ...
   *             <tt>columns() != other.columns() || rows() != other.rows()</tt>
   */
  IntMatrix2D copyFrom(IntMatrix2D other) {
    if (other == this) return this;
    checkShape(other);
    IntMatrix2D other_loc;
   ...
   */
  bool _haveSharedCells(IntMatrix2D other) {
   ...
   * @see #rowFlip()
   */
  IntMatrix2D columnFlip() {
    return (_view()._vColumnFlip()) as IntMatrix2D;
  }
   ...
   */
  IntMatrix2D dice() {
    return (_view()._vDice()) as IntMatrix2D;
  }
   ...
   *
   */
  IntMatrix2D part(int row, int column, int height, int width) {
    return (_view()._vPart(row, column, height, width)) as IntMatrix2D;
  }
   ...
   */
  IntMatrix2D rowFlip() {
    return (_view()._vRowFlip()) as IntMatrix2D;
  }
   ...
   *         final bool apply(IntMatrix1D m) { return m.get(0) &lt; threshold; }
   *      }
   *   );
   *
   ...
   *   matrix.viewSelection(
   *      new IntMatrix1DProcedure() {
   *         final bool apply(IntMatrix1D m) { return Math.sqrt(m.aggregate(F.plus,F.square) / m.length) &lt; threshold; }
   *      }
   *   );
   *
   * </pre>
   ...
   * @return the new view.
   */
  IntMatrix2D where(IntMatrix1DProcedure condition) {
   ...
   */
  IntMatrix2D select(Int32List rowIndexes, Int32List columnIndexes) {
    // check for "all"
    if (rowIndexes == null) {
  ...
  IntMatrix2D selectIterable(Set<Int32List> indexes) {
   ...
   * {@link IntSorting#sort(IntMatrix2D,int)}. For
   ...
   */
  IntMatrix2D strides(int rowStride, int columnStride) {
    return (_view()._vStrides(rowStride, columnStride)) as IntMatrix2D;
  }
   ...
   */
  /*IntMatrix1D mult(IntMatrix1D y, IntMatrix1D z) {
  ...
  IntMatrix1D mult(final IntMatrix1D y, IntMatrix1D z, [final int alpha = 1, int beta = null, final bool transposeA = false]) {
    ...
    IntMatrix1D z_loc;
    if (z == null) {
      z_loc = new DenseIntMatrix1D(this._rows);
    } else {
      z_loc = z;
    }
   ...
   */
  /*IntMatrix2D multiply(IntMatrix2D B, IntMatrix2D C) {
