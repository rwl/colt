 ...
 *
 */
abstract class IntMatrix1D extends AbstractMatrix1D {
   ...
   * @see IntFunctions
   */
  IntMatrix1D forEach(final ifunc.IntFunction f) {
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (_size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
      nthreads = Math.min(nthreads, _size);
      List<Future> futures = new List<Future>(nthreads);
   ...
   * @see IntFunctions
   */
  IntMatrix1D forEachWhere(final ifunc.IntProcedure cond, final ifunc.IntFunction f) {
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (_size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
      nthreads = Math.min(nthreads, _size);
      List<Future> futures = new List<Future>(nthreads);
   ...
   *             if <tt>size() != other.size()</tt>.
   */
  IntMatrix1D copyFrom(IntMatrix1D other) {
    if (other == this) return this;
    checkSize(other);
    IntMatrix1D other_loc;
   ...
   * @see IntFunctions
   */
  IntMatrix1D forEachWithRange(IntMatrix1D y, ifunc.IntIntFunction function, List<int> nonZeroIndexes) {
    checkSize(y);
   ...
   */
  IntMatrix1D flip() {
    return (_view()._vFlip()) as IntMatrix1D;
  }
   ...
   *
   */
  IntMatrix1D part(int index, int width) {
    return (_view()._vPart(index, width)) as IntMatrix1D;
  }
   ...
   * @return the new view.
   */
  IntMatrix1D where(ifunc.IntProcedure condition) {
   ...
   */
  IntMatrix1D select(Int32List indexes) {
    // check for "all"
    if (indexes == null) {
   ...
   *
   */
  IntMatrix1D strides(int stride) {
    return (_view()._vStrides(stride)) as IntMatrix1D;
  }
   ...
   */
  int dot(final IntMatrix1D y, [final int from = 0, int length = null]) {
    if (length == null) {
      length = _size;
    }
   ...
   * @return the sum of products.
   */
  int dotNonZero(IntMatrix1D y, Int32List nonZeroIndexes, [int from = 0, int length = null]) {
    if (length == null) {
      length = _size;
    }
   ...
   * @return the sum of products.
   */
  /*int dotNonZero(IntMatrix1D y, Int32List nonZeroIndexes) {
