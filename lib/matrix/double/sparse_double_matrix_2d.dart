 ...
 */
class SparseDoubleMatrix2D extends DoubleMatrix2D {
   ...
   *            numerical value
   */
  factory SparseDoubleMatrix2D.value(int rows, int columns, Int32List rowIndexes, Int32List columnIndexes, double value) {
    /*try {
            _setUp(rows, columns);
        } on ArgumentError catch (exc) { // we can hold rows*columns>Integer.MAX_VALUE cells !
    ...
    return new SparseDoubleMatrix2D(rows, columns).._insert(rowIndexes, columnIndexes, value);
  }
   ...
   *            numerical values
   */
  factory SparseDoubleMatrix2D.values(int rows, int columns, Int32List rowIndexes, Int32List columnIndexes, Float64List values) {
    /*try {
            _setUp(rows, columns);
        } on ArgumentError catch (exc) { // we can hold rows*columns>Integer.MAX_VALUE cells !
            if (!"matrix too large".equals(exc.getMessage()))
    ...
    return new SparseDoubleMatrix2D(rows, columns).._insertValues(rowIndexes, columnIndexes, values);
  }
   ...
   * @throws IOException
   */
  /*factory SparseDoubleMatrix2D.read(MatrixVectorReader reader) {
  ...
  /*DoubleMatrix2D assign(func.DoubleFunction function) {
   ...
   */
  SparseDoubleMatrix2D assignIndexValueFunc(final Int32List rowIndexes, final Int32List columnIndexes, final double value, final func.DoubleDoubleFunction function) {
   ...
   * @return <tt>this</tt> (for convenience only).
   */
  SparseDoubleMatrix2D assignIndexValuesFunc(final Int32List rowIndexes, final Int32List columnIndexes, final Float64List values, final func.DoubleDoubleFunction function) {
    int size = rowIndexes.length;
    if (function == func.plus) { // x[i] = x[i] + y[i]
      for (int i = 0; i < size; i++) {
   ...
   */
  /*SparseCCMDoubleMatrix2D getColumnCompressedModified() {
    SparseCCMDoubleMatrix2D A = new SparseCCMDoubleMatrix2D(_rows, _columns);
    int nnz = cardinality();
    final keys = _elements.keys;
    final values = _elements.values;
    for (int i = 0; i < nnz; i++) {
   ...
   */
  /*SparseRCMDoubleMatrix2D getRowCompressedModified() {
    SparseRCMDoubleMatrix2D A = new SparseRCMDoubleMatrix2D(_rows, _columns);
    int nnz = cardinality();
    final keys = _elements.keys;
    final values = _elements.values;
    for (int i = 0; i < nnz; i++) {
  ...
  DoubleMatrix1D _like1D(int size, int offset, int stride) {
    return new SparseDoubleMatrix1D(size, this._elements, offset, stride);
  }
...
class SelectedSparseDoubleMatrix2D extends DoubleMatrix2D {
   ...
   * @param offset
   */
  factory SelectedSparseDoubleMatrix2D.from(Map<int, double> elements, Int32List rowOffsets, Int32List columnOffsets, int offset) {
    return new SelectedSparseDoubleMatrix2D(rowOffsets.length, columnOffsets.length, elements, 0, 0, 1, 1, rowOffsets, columnOffsets, offset);
  }
   ...
   * @param offset
   */
  SelectedSparseDoubleMatrix2D(int rows, int columns, Map<int, double> elements, int rowZero, int columnZero, int rowStride, int columnStride, Int32List rowOffsets, Int32List columnOffsets, int offset) {
   ...
   */
  DoubleMatrix2D _viewSelectionLike(Int32List rowOffsets, Int32List columnOffsets) {
    return new SelectedSparseDoubleMatrix2D.from(this._elements, rowOffsets, columnOffsets, this._offset);
  }

  Object clone() {
    return new SelectedSparseDoubleMatrix2D(_rows, _columns, _elements, _rowZero, _columnZero,
