...
class DenseDoubleMatrix extends DoubleMatrix {
   ...
   *             .
   */
  factory DenseDoubleMatrix.fromList(List<Float64List> values) {
    return new DenseDoubleMatrix(values.length, values.length == 0 ? 0 : values[0].length)
      ..setAll2D(values);
  }
   ...
   */
  DenseDoubleMatrix(int rows, int columns, [Float64List elements = null, int rowZero = 0, int columnZero = 0, int rowStride = null, int columnStride = 1, bool isView = false]) {
    if (elements == null) {
   ...
   * @throws IOException
   */
  /*factory DenseDoubleMatrix.read(MatrixVectorReader reader) {//throws IOException {
    MatrixInfo info;
    if (reader.hasInfo()) {
  ...
  double reduceMatrix(final DoubleMatrix other, DoubleDoubleFunction aggr, DoubleDoubleFunction f) {
    if (!(other is DenseDoubleMatrix)) {
  ...
  DoubleMatrix forEach(DoubleFunction function) {
  ...
  DoubleMatrix forEachWhere(DoubleProcedure cond, func.DoubleFunction function) {
    final int zero = index(0, 0);
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
      nthreads = Math.min(nthreads, _rows);
  ...
  DoubleMatrix fillWhere(DoubleProcedure cond, final double value) {
    final int zero = index(0, 0);
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
      nthreads = Math.min(nthreads, _rows);
  ...
  DoubleMatrix fill(final double value) {
    final Float64List elems = this._elements;
    final int zero = index(0, 0);
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
  ...
  DoubleMatrix copyFrom(final DoubleMatrix source) {
    ...
    if (_haveSharedCells(other)) {
      ...
      if (!(c is DenseDoubleMatrix)) { // should not happen
  ...
  DoubleMatrix forEachMatrix(final DoubleMatrix y, DoubleDoubleFunction function) {
    // overriden for performance only
  ...
  DoubleMatrix forEachMatrixRange(final DoubleMatrix y, DoubleDoubleFunction function,  /*IntArrayList*/List<int> rowList,  /*IntArrayList*/List<int> columnList) {
    checkShape(y);
    final int size = rowList.length;
    final List<int> rowElements = rowList;//.elements();
    final List<int> columnElements = columnList;//.elements();
  ...
  DoubleMatrix forEachNonZero(func.IntIntDoubleFunction function) {
    final int zero = this.index(0, 0);
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
      nthreads = Math.min(nthreads, _rows);
   ...
   */
  /*DenseColumnDoubleMatrix getColumnMajor() {
    DenseColumnDoubleMatrix R = new DenseColumnDoubleMatrix(_rows, _columns);
  ...
  /*void zAssign8Neighbors(DoubleMatrix B, func.Double9Function function) {
  ...
  DoubleVector mult(final DoubleVector y, DoubleVector z, [final double alpha = 1.0, final double beta = 0.0, final bool transposeA = false]) {
    if (transposeA) {
      return dice().mult(y, z, alpha, beta, false);
    }
    if (z == null) {
      z = new DenseDoubleVector(_rows);
    }
    if (!(y is DenseDoubleVector && z is DenseDoubleVector)) return super.mult(y, z, alpha, beta, transposeA);
  ...
  DoubleMatrix multiply(final DoubleMatrix B, DoubleMatrix C, [final double alpha = 1.0, final double beta = 0.0, final bool transposeA = false, bool transposeB = false]) {
    ...
    if (C == null) {
      C = new DenseDoubleMatrix(rowsC, columnsC);
    }
    ...
    */
    ...
    if (B is SparseDoubleMatrix || B is SparseRCDoubleMatrix) {
    ...
    for (int i = 0; i < noOfTasks; i++) {
      ...
      /*subTasks[i] = ConcurrencyUtils.submit(() {
        (AA as DenseDoubleMatrix).zMultSequential(BB, CC, alpha, beta, transposeA, transposeB);
      });*/
      (AA as DenseDoubleMatrix).multiplySequential(BB, CC, alpha, beta, transposeA, transposeB);
    }
  ...
  DoubleMatrix multiplySequential(DoubleMatrix B, DoubleMatrix C, double alpha, double beta, bool transposeA, bool transposeB) {
    if (transposeA) {
      return dice().multiply(B, C, alpha, beta, false, transposeB);
    }
    if (B is SparseDoubleMatrix || B is SparseRCDoubleMatrix || B is SparseCCDoubleMatrix) {
      // exploit quick sparse mult
      // A*B = (B' * A')'
      if (C == null) {
        return B.multiply(this, null, alpha, beta, !transposeB, true).dice();
  ...
  DoubleVector _like1D(int size, int zero, int stride) {
    return new DenseDoubleVector(size, this._elements, zero, stride, true);
  }
  ...
  Object clone() {
    return new DenseDoubleMatrix(_rows, _columns, _elements, _rowZero, _columnZero, _rowStride, _columnStride, !_isNoView);
  }
}
...
class SelectedDenseDoubleMatrix extends DoubleMatrix {
  ...
  DoubleVector column(int column) {
    ...
    return new SelectedDenseDoubleVector(viewSize, this._elements, viewZero, viewStride, viewOffsets, viewOffset);
  }
  ...
  Object clone() {
    return new SelectedDenseDoubleMatrix(_rows, _columns, _elements, _rowZero, _columnZero, _rowStride, _columnStride, _rowOffsets, _columnOffsets, _offset, !_isNoView);
  }
}
