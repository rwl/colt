...
//    }
   ...
   *             if <tt>size() != other.size()</tt>.
   */
  void swap(final AbstractDoubleVector other) {
    checkSize(other);
    /*int nthreads = ConcurrencyUtils.getNumberOfThreads();
    if ((nthreads > 1) && (_size >= ConcurrencyUtils.getThreadsBeginN_1D())) {
      nthreads = Math.min(nthreads, _size);
   ...
   */
  double dot(final AbstractDoubleVector y, [final int from = 0, int length = null]) {
    if (length == null) {
      length = _size;
    }
    if (from < 0 || length <= 0) {
   ...
   */
  AbstractDoubleVector _getContent() {
    return this;
  }
  ...
  AbstractDoubleVector operator >(double b) {
    return this.copy()..forEach(func.greaterThan(b));
  }
