 ...
 */
void checkRectangular(AbstractDoubleMatrix A) {
  if (A.rows < A.columns) {
 ...
 *             if <tt>A.rows() != A.columns()</tt>.
 */
void checkSquare(AbstractDoubleMatrix A) {
  if (A.rows != A.columns) {
...
void checkDense(AbstractDoubleVector A) {
  if (!(A is DoubleVector)) {
    throw new ArgumentError("Matrix must be dense");
  }
}

void checkSparse(AbstractDoubleVector A) {
  if (!(A is SparseDoubleVector)) {
    throw new ArgumentError("Matrix must be sparse");
  }
}

void checkSparse2D(AbstractDoubleMatrix A) {
 ...
 */
double density(AbstractDoubleMatrix A) {
 ...
 *         otherwise.
 */
bool equals(final AbstractDoubleVector A, final double value, [double epsilon = EPSILON]) {
  if (A == null) {
    return false;
  }
  int size = A.length;
 ...
 *         otherwise.
 */
bool equalsVector(final AbstractDoubleVector A, final AbstractDoubleVector B, [double epsilon = EPSILON]) {
  if (identical(A, B)) {
    return true;
  }
  if (!(A != null && B != null)) {
 ...
 *         otherwise.
 */
bool equalsMatrixValue(final AbstractDoubleMatrix A, final double value, [double epsilon = EPSILON]) {
  if (A == null) {
    return false;
  }
 ...
 *         otherwise.
 */
bool equalsMatrix(final AbstractDoubleMatrix A, final AbstractDoubleMatrix B, [double epsilon = EPSILON]) {
  if (identical(A, B)) {
    return true;
  }
  if (!(A != null && B != null)) {
 ...
 *             if <tt>!isSquare(A)</tt>.
 */
void generateNonSingular(AbstractDoubleMatrix A) {
  checkSquare(A);
 ...
 */
bool isDiagonal(AbstractDoubleMatrix A, [double epsilon = EPSILON]) {
  int rows = A.rows;
 ...
 */
bool isDiagonallyDominantByColumn(AbstractDoubleMatrix A) {
  //cern.jet.math.tdouble.DoubleFunctions F = cern.jet.math.tdouble.DoubleFunctions.functions;
  int min = Math.min(A.rows, A.columns);
  for (int i = min; --i >= 0; ) {
 ...
 * Note: Ignores tolerance.
 */
bool isDiagonallyDominantByRow(AbstractDoubleMatrix A) {
  //cern.jet.math.tdouble.DoubleFunctions F = cern.jet.math.tdouble.DoubleFunctions.functions;
  int min = Math.min(A.rows, A.columns);
  for (int i = min; --i >= 0; ) {
    double diag = (A.get(i, i)).abs();
 ...
 */
bool isIdentity(AbstractDoubleMatrix A, [double epsilon = EPSILON]) {
  int rows = A.rows;
  int columns = A.columns;
  for (int row = rows; --row >= 0; ) {
    for (int column = columns; --column >= 0; ) {
 ...
 */
bool isLowerBidiagonal(AbstractDoubleMatrix A, [double epsilon = EPSILON]) {
  int rows = A.rows;
  int columns = A.columns;
  for (int row = rows; --row >= 0; ) {
    for (int column = columns; --column >= 0; ) {
 ...
 */
bool isLowerTriangular(AbstractDoubleMatrix A, [double epsilon = EPSILON]) {
  int rows = A.rows;
  int columns = A.columns;
  for (int column = columns; --column >= 0; ) {
 ...
 * Note: Ignores tolerance.
 */
bool isNonNegative(AbstractDoubleMatrix A) {
  int rows = A.rows;
  int columns = A.columns;
  for (int row = rows; --row >= 0; ) {
    for (int column = columns; --column >= 0; ) {
 ...
 *             if <tt>!isSquare(A)</tt>.
 */
bool isOrthogonal(AbstractDoubleMatrix A) {
  checkSquare(A);
 ...
 * Note: Ignores tolerance.
 */
bool isPositive(AbstractDoubleMatrix A) {
  int rows = A.rows;
  int columns = A.columns;
  for (int row = rows; --row >= 0; ) {
    for (int column = columns; --column >= 0; ) {
 ...
 *             if <tt>!isSquare(A)</tt>.
 */
bool isSkewSymmetric(AbstractDoubleMatrix A, [double epsilon = EPSILON]) {
  checkSquare(A);
  int rows = A.rows;
  for (int row = rows; --row >= 0; ) {
 ...
 * and columns.
 */
bool isSquare(AbstractDoubleMatrix A) {
 ...
 * be square.
 */
bool isStrictlyLowerTriangular(AbstractDoubleMatrix A, [double epsilon = EPSILON]) {
  int rows = A.rows;
  int columns = A.columns;
  for (int column = columns; --column >= 0; ) {
 ...
 */
bool isStrictlyTriangular(AbstractDoubleMatrix A, [double epsilon = EPSILON]) {
 ...
 * be square.
 */
bool isStrictlyUpperTriangular(AbstractDoubleMatrix A, [double epsilon = EPSILON]) {
  int rows = A.rows;
  int columns = A.columns;
  for (int column = columns; --column >= 0; ) {
 ...
 *             if <tt>!isSquare(A)</tt>.
 */
bool isSymmetric(AbstractDoubleMatrix A) {
  checkSquare(A);
 ...
 */
bool isTriangular(AbstractDoubleMatrix A) {
 ...
 */
bool isTridiagonal(AbstractDoubleMatrix A, [double epsilon = EPSILON]) {
  int rows = A.rows;
  int columns = A.columns;
  for (int row = rows; --row >= 0; ) {
    for (int column = columns; --column >= 0; ) {
 ...
 */
bool isUnitTriangular(AbstractDoubleMatrix A, [double epsilon = EPSILON]) {
  if (!isTriangular(A)) return false;
 ...
 * unless <tt>i==j || i==j-1</tt>. Matrix may but need not be square.
 */
bool isUpperBidiagonal(AbstractDoubleMatrix A, [double epsilon = EPSILON]) {
  int rows = A.rows;
  int columns = A.columns;
  for (int row = rows; --row >= 0; ) {
    for (int column = columns; --column >= 0; ) {
 ...
 * whenever <tt>i &gt; j</tt>. Matrix may but need not be square.
 */
bool isUpperTriangular(AbstractDoubleMatrix A, [double epsilon = EPSILON]) {
  int rows = A.rows;
  int columns = A.columns;
  for (int column = columns; --column >= 0; ) {
    for (int row = rows; --row > column; ) {
 ...
 */
bool isZero(AbstractDoubleMatrix A) {
 ...
 */
int lowerBandwidth(AbstractDoubleMatrix A, [double epsilon = EPSILON]) {
  checkSquare(A);
  int rows = A.rows;
 ...
 * @see #upperBandwidth(DoubleMatrix)
 */
int semiBandwidth(AbstractDoubleMatrix A, [double epsilon = EPSILON]) {
  checkSquare(A);
  int rows = A.rows;
 ...
 * </pre>
 */
String toString2D(AbstractDoubleMatrix A) {
 ...
 * @see #lowerBandwidth(DoubleMatrix)
 */
int upperBandwidth(AbstractDoubleMatrix A, [double epsilon = EPSILON]) {
  checkSquare(A);
  int rows = A.rows;
