...
//    }
    ...
    /**
     */
    static String _get(ObjectArrayList list, int index) {
        return (list.get(index) as String);
    }
    ...
    bool isDiagonal(DoubleMatrix2D A) {
        ...
        for (int row = rows; --row >= 0;) {
            for (int column = columns; --column >= 0;) {
                if (row != column && !((A.getQuick(row, column)).abs() <= epsilon))
                    return false;
            }
        }
        return true;
    ...
    bool isDiagonallyDominantByColumn(DoubleMatrix2D A) {
        ...
        for (int i = min; --i >= 0;) {
            ...
            diag += diag;
            if (diag <= A.viewColumn(i).aggregate(plus, abs))
                return false;
        }
        return true;
    }
    ...
    bool isDiagonallyDominantByRow(DoubleMatrix2D A) {
        ...
        for (int i = min; --i >= 0;) {
            double diag = (A.getQuick(i, i)).abs();
            diag += diag;
            if (diag <= A.viewRow(i).aggregate(plus, abs))
                return false;
        }
        return true;
    }
        ...
        for (int row = rows; --row >= 0;) {
            for (int column = columns; --column >= 0;) {
                if (!(row == column || row == column + 1)) {
                    if (!((A.getQuick(row, column)).abs() <= epsilon))
                        return false;
                }
            }
        }
    ...
    bool isOrthogonal(DoubleMatrix2D A) {
        checkSquare(A);
        return equals(A.zMult2D(A, null, 1, 0, false, true), DoubleFactory2D.dense.identity(A
                .rows()));
    }
    ...
    bool isSkewSymmetric(DoubleMatrix2D A) {
        ...
        for (int row = rows; --row >= 0;) {
            for (int column = rows; --column >= 0;) {
                if (!((A.getQuick(row, column) + A.getQuick(column, row)).abs() <= epsilon))
                    return false;
            }
        }
        return true;
    ...
    bool isUnitTriangular(DoubleMatrix2D A) {
        ...
        double epsilon = tolerance();
        for (int i = Math.min(A.rows(), A.columns()); --i >= 0;) {
            if (!((1 - A.getQuick(i, i)).abs() <= epsilon))
                return false;
        }
        return true;
    }
