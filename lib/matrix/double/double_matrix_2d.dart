...
abstract class DoubleMatrix2D extends AbstractMatrix2D {
    ...
    bool equals(Object obj) {
        ...
        return cern.colt.matrix.tdouble.algo.DoubleProperty.DEFAULT.equals(this, obj as DoubleMatrix2D);
    }
     ...
     * @return <tt>this</tt> (for convenience only).
     */
    DoubleMatrix2D forEachNonZero(final IntIntDoubleFunction function) {
        int nthreads = ConcurrencyUtils.getNumberOfThreads();
        if ((nthreads > 1) && (size() >= ConcurrencyUtils.getThreadsBeginN_2D())) {
            nthreads = Math.min(nthreads, _rows);
            List<Future> futures = new List<Future>(nthreads);
    ...
    DoubleMatrix1D viewColumn(int column) {
        ...
        int viewZero = index(0, column) as int;
    ...
    DoubleMatrix2D viewColumnFlip() {
        return (DoubleMatrix2D) (_view()._vColumnFlip());
    }
    ...
    DoubleMatrix2D viewDice() {
        return (DoubleMatrix2D) (_view()._vDice());
    }
    ...
    DoubleMatrix2D viewPart(int row, int column, int height, int width) {
        return (DoubleMatrix2D) (_view()._vPart(row, column, height, width));
    }
    ...
    DoubleMatrix1D viewRow(int row) {
        ...
        int viewZero = index(row, 0) as int;
    ...
    DoubleMatrix2D viewRowFlip() {
        return (DoubleMatrix2D) (_view()._vRowFlip());
    }
    ...
    DoubleMatrix2D viewSelectionSet(Set<List<int>> indexes) {
        ...
        int idx = 0;
        for (Iterator<List<int>> iterator = indexes.iterator(); iterator.hasNext();) {
            List<int> _is = iterator.next();
    ...
    DoubleMatrix2D viewStrides(int rowStride, int columnStride) {
        return (DoubleMatrix2D) (_view()._vStrides(rowStride, columnStride));
    }
