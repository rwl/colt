 ...
 * picture.
 */
abstract class AbstractIntList extends AbstractList {//implements cern.colt.buffer.tint.IntBufferConsumer {
     ...
     */
    void addAllOf(IntArrayList other) {
     ...
     *                ).
     */
    void addAllOfFromTo(AbstractIntList other, int from, int to) {
     ...
     *                <tt>index &lt; 0 || index &gt; size()</tt>).
     */
    void beforeInsertAllOfFromTo(int index, AbstractIntList other, int from, int to) {
        int length = to - from + 1;
        this._beforeInsertDummies(index, length);
    ...
    List<int> get elements {
        List<int> myElements = new List<int>(_size);
        for (int i = _size; --i >= 0;)
    ...
    /*AbstractIntList*/void set elements(List<int> elements) {
        clear();
        addAllOfFromTo(new IntArrayList.from(elements), 0, elements.length - 1);
        return this;
    }
    ...
    bool equals(Object otherObj) { // delta
        ...
        if (otherObj == null)
            return false;
        AbstractIntList other = otherObj as AbstractIntList;
        if (size() != other.size())
            return false;
     ...
     *            receiver.
     */
    void fillFromToWith(int from, int to, int val) {
        _checkRangeFromTo(from, to, this._size);
        for (int i = from; i <= to;)
            _setQuick(i++, val);
    }
     ...
     */
    bool forEach(IntProcedure procedure) {
        for (int i = 0; i < _size;)
    ...
    void mergeSortFromTo(int from, int to) {
        int mySize = size();
        _checkRangeFromTo(from, to, mySize);

        List<int> myElements = elements();
     ...
     *                ).
     */
    void mergeSortFromToCompare(int from, int to, IntComparator c) {
        int mySize = size();
        _checkRangeFromTo(from, to, mySize);
     ...
     *                ).
     */
    void quickSortFromToCompare(int from, int to, IntComparator c) {
        int mySize = size();
        _checkRangeFromTo(from, to, mySize);
     ...
     *         call.
     */
    bool removeAll(AbstractIntList other) {
        if (other.size() == 0)
        ...
        for (int i = 0; i < _size; i++) {
            if (other.indexOfFromTo(getQuick(i), 0, limit) < 0)
                _setQuick(j++, getQuick(i));
        }

        bool modified = (j != _size);
        setSize(j);
        return modified;
    }
    ...
    void replaceFromToWithFrom(int from, int to, AbstractIntList other, int otherFrom) {
        ...
        if (length > 0) {
            ...
            if (from <= otherFrom) {
                for (; --length >= 0;)
                    _setQuick(from++, other.getQuick(otherFrom++));
            } else {
                int otherTo = otherFrom + length - 1;
                for (; --length >= 0;)
                    _setQuick(to--, other.getQuick(otherTo--));
            }
     ...
     * </pre>
     */
    void replaceFromToWithFromTo(int from, int to, AbstractIntList other, int otherFrom, int otherTo) {
        ...
        if (diff > 0) {
            _beforeInsertDummies(theLast + 1, diff);
        } else {
            if (diff < 0) {
     ...
     *         call.
     */
    bool retainAll(AbstractIntList other) {
        if (other.size() == 0) {
            if (_size == 0)
                return false;
            setSize(0);
    ...
    void shuffleFromTo(int from, int to) {
        ...
        DoubleUniform gen = new DoubleUniform(
                new engine.DRand(new java.util.Date()));
        for (int i = from; i < to; i++) {
            int random = gen.nextIntFromTo(i, to);

            // swap(i, random)
            int tmpElement = getQuick(random);
            _setQuick(random, getQuick(i));
            _setQuick(i, tmpElement);
        }
    }
     ...
     */
    AbstractIntList times(int times) {
