...
typedef DoubleVector VectorFn(int);
typedef DoubleMatrix MatrixFn(int rows, int columns);

/// `C = A||B`; Constructs a new matrix which is the concatenation of two
/// other matrices.
DoubleVector append(DoubleVector A, DoubleVector B, VectorFn make) {
  // concatenate
  DoubleVector matrix = make(A.size + B.size);
...
/// are copied.
DoubleVector concat(List<DoubleVector> parts, VectorFn make) {
  if (parts.length == 0) {
    return make(0);
  }
...
/// given value.
DoubleVector fill(int size, double initialValue, VectorFn make) {
  return make(size)..fill(initialValue);
}

/// Constructs a matrix with uniformly distributed values in `(0,1)`
/// (exclusive).
DoubleVector random(int size, VectorFn make) {
...
///     0 1 2 6
///     3 4 5 8
DoubleMatrix appendColumn(DoubleMatrix A, DoubleVector b, MatrixFn make) {
...
///     0 1 2 6 7
///     3 4 5 8 9
DoubleMatrix appendColumns(DoubleMatrix A, DoubleMatrix B, MatrixFn make) {
  // force both to have maximal shared number of rows.
  if (B.rows > A.rows) {
...
///     4 5
///     6 7
DoubleMatrix appendRow(DoubleMatrix A, DoubleVector b, MatrixFn make) {
...
///     6 7
///     8 9
DoubleMatrix appendRows(DoubleMatrix A, DoubleMatrix B, MatrixFn make) {
  // force both to have maximal shared number of columns.
...
DoubleMatrix compose(List<List<DoubleMatrix>> parts, MatrixFn make) {
...
///     0 4 0
///     0 0 3
DoubleMatrix diagonalMatrix(DoubleVector vector, MatrixFn make) {
...
/// are copied.
DoubleMatrix makeColumn(Float64List values, int rows, MatrixFn make) {
...
/// Constructs a matrix with uniformly distributed values in `(0,1)`
/// (exclusive).
DoubleMatrix randomMatrix(int rows, int columns, MatrixFn make) {
